...
;; return: true if start cell is closer to cell1
;;         false if start cell is closer to cell2
(define (iscloser? start-cell cell1 cell2 board)
  (< (distance start-cell-number cell1-number board)
     (distance start-cell-number cell2-number board))
  )
...
(define (move->queued-move origin destination board)
  (if (= origin destination)
      (error "MOVE->QUEUED-MOVE - origin cannot be equal to destination")
      (list 'queued-move
